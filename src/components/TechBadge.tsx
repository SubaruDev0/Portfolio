...
export default function TechBadge({ name, showName = true, className = "" }: TechBadgeProps) {
  ...
  return (
    <div 
      className={`flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-xl group/tech hover:border-white/20 hover:bg-white/10 transition-all cursor-default relative ${className}`}
    >
      <div className="w-5 h-5 flex items-center justify-center text-orange-500">
        {error ? (
          ...
          <img 
            src={iconUrl} 
            alt={name}
            className={`w-full h-full object-contain opacity-70 group-hover/tech:opacity-100 group-hover/tech:scale-110 transition-all ${['java', 'videojuego', 'videojuegos'].includes(name.toLowerCase()) ? 'brightness-0 invert' : ''}`}
      ...
      {showName ? (
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 group-hover/tech:text-white transition-colors">
          {name}
        </span>
      ) : (
        /* Tooltip para cuando no hay nombre visible (en las tarjetas) */
        <span className="absolute -top-10 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 border border-white/10 rounded text-[10px] uppercase tracking-[0.2em] text-white opacity-0 group-hover/tech:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-xl">
          {name}
        </span>
      )}
    </div>
